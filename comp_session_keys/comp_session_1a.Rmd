...
subtitle: 'Meet R, RStudio, R Markdown, basic math operations, and our first function'
output: 
  html_document:
...
R Markdown is a document where we can combine markdown (plain text with added formatting), code, and outputs all in one place - which makes it an incredible tool for reproducibility. Let's make one (File > R Markdown), and learn by doing. 

In your new R Markdown document, let's add:
...
- Knitting: When you *knit* an R Markdown document, all content is first converted to plain markdown, then converted to a file output type you select (the default, and the one we'll use most often is HTML)
