...
subtitle: 'R Projects, more markdown, creating data, more functions and a ggplot2 graph'
output: 
  html_document:
...
- File > New project (or the blue cube)
...
- Make a new R Markdown file in your project, then follow along (adding notes in markdown) with the rest of the session
...
Attach the tidyverse in the setup chunk of your R Markdown document using `library(tidyverse)`. Note: you should already have the package installed on your computer - if not, you'll need to do that first. 
...
## No precious objects or outputs! 

- Save your .Rmd, which lives in your R project. 
...
- In the Files tab of RStudio, click on the .Rmd you saved
- Use Cmd + Option + R to run all code in your .Rmd
- Check to see that all objects and outputs are automatically reproduced

## End interactive session 1
