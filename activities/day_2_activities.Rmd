...
3. For the function above, what is the slope in the $p$ direction at $T = 1.9, k = 2.4$?
...
- Render (save your .qmd as `eds212_day2_activities.Rmd`, or follow a naming convention you've decided on)
...
- In a new code chunk, create a `ggplot` graph of penguin bill length versus bill depth (using the `penguins` dataset in the `palmerpenguins` package), with point color dependent on species. You should attach the `tidyverse` and `palmerpenguins` package in your .qmd setup chunk. Update labels and add a title.

- In a new code chunk, make a jitter plot of flipper length by species (species on the x-axis, flipper length on the y-axis). Update axis labels and add a title. 
